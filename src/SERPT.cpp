/***
* SERPT.cpp - implementation of SERPT Scheduler class
*
*		Written by Mithun P
*		Last Updated : July 2022
* Purpose:
*       This file contains the implementation of the SERPT Scheduler class.
*
****/
...
bool SERPT::arrival_handler(ArrivalEvent* e) {
	Job* top = NULL;
	if (!jobs_q.empty())
		top = jobs_q.top();
	...
	// If the new job is on top of SERPT queue, the there has been a context switch
...
Job* SERPT::departure_handler(DepartureEvent* e) {
...
bool SERPT::speedchange_handler(SpeedChangeEvent* e) {
	// SERPT doesn't care about the speed change.
	return false;
}
...
unsigned long SERPT::getExJob(double time) {
	if (jobs_q.empty()) {
...
Event* SERPT::nextDeparture(double speed, double time) {
	if (jobs_q.empty()) {
		yLog::logtime(DEBUGLOG, __FUNCTION__, "There is no job in the system.");
		return NULL;
	}
...
void SERPT::updatePeriod(double time1, double time2, double speed, DESLogger* logger, PowerFunction* P) {
	...
	if (jobs_q.empty())
		...
		// TOFIX:
				// Update the record for the updatee job
		if (approximatelyEqual(updatee->estimatedremsize_m - takeout, 0))
			updatee->estimatedremsize_m = 0;
		else if (definitelyGreaterThan(takeout, updatee->estimatedremsize_m)) {
...
Event* SERPT::bonusevent_handler(SchedulerEvent*, double speed) {
	// Anything to do?
	return NULL;
}
...
Event* SERPT::nextScheduler(double speed, double time) {
	// Anything to do?
	return NULL;
}
