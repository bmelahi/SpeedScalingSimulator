...
#define _PSEPT_h_
/*----------------------------------------------------------------------
* PSEPT.h - definitions/declarations for class PSEPT(Preemptive SEPT)
*
*       Written By Mithun P
*		Last Updated: July 2022
*
*Purpose:
*       This file defines the class PSEPT(Preemptive SEPT), with its members and methods.
*
*///---------------------------------------------------------------------
...
public:
	class CompareJobPSEPT {
	public:
		// Returns false if j1 should run earlier than j2 
		bool operator()(const Job* j1, const Job* j2) const {
			if (approximatelyEqual(j1->getEstimatedSize(), j2->getEstimatedSize())) {
				return definitelyGreaterThan(j1->getArrival(), j2->getArrival());
			}
			else
				return definitelyGreaterThan(j1->getEstimatedSize(), j2->getEstimatedSize());
		}
	};

private:
	std::priority_queue<Job*, std::vector<Job*>, CompareJobPSEPT> jobs_q;	// The queue of jobs in the system
	//Job* current_job_m;
