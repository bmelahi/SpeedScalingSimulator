/***
* FSPEI.cpp - implementation of SRPT Scheduler class
*
*       Written by Maryam Elahi
*		Last (remembered) updated: AUG, 2016
*
* Purpose:
...
Job * FSPEI::departure_handler(DepartureEvent * e) {
	...
	if (exJob_m == NULL && !jobs_q.empty())
		yLog::logtime(ERRORLOG, __FUNCTION__,
		"No job in FSPEI matches the next job from virtual PS: %d", nextJob);
...
bool FSPEI::speedchange_handler(SpeedChangeEvent * e) {
...
unsigned long FSPEI::getExJob(double time) {
	// Check if exJob_m is NULL
	if (exJob_m == NULL) {
		if (!jobs_q.empty())
			yLog::logtime(ERRORLOG, __FUNCTION__, "No job under execution, while there is at least one job in the system!");
		...
		//for (set<Job *, CompareJobFSPEI>::iterator it = jobs_q.begin(); it != jobs_q.end(); it++) {
		//	if (!(*it)->isFinished_m) {
...
Event * FSPEI::nextDeparture(double speed, double time) {
	// Check if exJob_m is NULL
	if (exJob_m == NULL) {
		if (!jobs_q.empty())
			yLog::logtime(ERRORLOG, __FUNCTION__, "No job under execution, while there is at least one job in the system!");
...
void FSPEI::updatePeriod(double time1, double time2, double speed, DESLogger * logger, PowerFunction * P) {
...
Event * FSPEI::bonusevent_handler(SchedulerEvent * e, double speed) {
	...
	// Check if the job that has completed under PS has already finished under FSP. 
...
Event * FSPEI::nextScheduler(double speed, double time) {
