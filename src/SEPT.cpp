/***
* SEPT.cpp - implementation of SEPT(Non Preemptive) Scheduler class
*
*		Written by Mithun P
*		Last Updated: July 2022
*
* Purpose:
*       This file contains the implementation of the SEPT(Non Preemptive) Scheduler class.
*
***/
...
bool SEPT::arrival_handler(ArrivalEvent* e) {
...
Job* SEPT::departure_handler(DepartureEvent* e) {
...
bool SEPT::speedchange_handler(SpeedChangeEvent* e) {
	// SEPT doesn't care about the speed change.
	return false;
}
...
unsigned long SEPT::getExJob(double time) {
...
Event* SEPT::nextDeparture(double speed, double time) {
	if (NULL == current_job_m) {
		yLog::logtime(DEBUGLOG, __FUNCTION__, "There is no job in the system.");
		if (!jobs_q.empty())
			yLog::logtime(ERRORLOG, __FUNCTION__, "There is no job currently under execution but the queue is not empty!");
...
void SEPT::updatePeriod(double time1, double time2, double speed, DESLogger* logger, PowerFunction* P) {
...
Event* SEPT::bonusevent_handler(SchedulerEvent*, double speed) {
	// Anything to do?
	return NULL;
}
...
Event* SEPT::nextScheduler(double speed, double time) {
	// Anything to do?
	return NULL;
}
