...
bool FSP::arrival_handler(ArrivalEvent * e) {
...
Job * FSP::departure_handler(DepartureEvent * e) {
	...
	if (nextJob == INVALIDID && !jobs_q.empty())
		yLog::logtime(ERRORLOG, __FUNCTION__, 
			"PS getExJob returned INVALIDID, while there are %d jobs in FSP!", jobs_q.size());		
	...
	if (exJob_m == NULL && !jobs_q.empty())
		yLog::logtime(ERRORLOG, __FUNCTION__, 
			"No job in FSP matches the next job from virtual PS: %d", nextJob);
...
bool FSP::speedchange_handler(SpeedChangeEvent * e) {
...
unsigned long FSP::getExJob(double time) {
	// Check if exJob_m is NULL
	if (exJob_m == NULL) {
		if(!jobs_q.empty())
			yLog::logtime(ERRORLOG, __FUNCTION__, "No job under execution, while there is at least one job in the system!");
		...
		//for (set<Job *, CompareJobFSP>::iterator it = jobs_q.begin(); it != jobs_q.end(); it++) {
		//	if (!(*it)->isFinished_m) {
...
Event * FSP::nextDeparture(double speed, double time) {
	// Check if exJob_m is NULL
	if (exJob_m == NULL) {
		if(!jobs_q.empty())
			yLog::logtime(ERRORLOG, __FUNCTION__, "No job under execution, while there is at least one job in the system!");
...
void FSP::updatePeriod(double time1, double time2, double speed, DESLogger * logger, PowerFunction * P) {
...
Event * FSP::bonusevent_handler(SchedulerEvent * e, double speed) {
...
Event * FSP::nextScheduler(double speed, double time) {
